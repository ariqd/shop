        ...
        $(document).ready(function () {
            ...
            $('#customers').change(function () {
                if ($(this).val() != 0) {
                    ...
                    $.ajax({
                        url: "{!! url('admin/sales/search-customer') !!}/" + $(this).val(),
                        method: "get",
                        beforeSend: function () {
                            $('.loading').show();
                        },
            ...
            $('#search').change(function () {
                $.ajax({
                    url: "{!! url('admin/sales/search') !!}/" + $("#search").val(),
                    method: "get",
                    beforeSend: function () {
                        $('.loading').show();
                    },
...
<div class="tb-content tb-style1 pb-5">
    ...
    <div class="container-fluid">
                ...
                <div class="col-12">
                    <div class="tb-alert">
                        <div class="d-flex justify-content-between">
                            <div class="py-1">
                                <i class="fa fa-exclamation-circle"></i> Customer belum melunasi pembelian. Harap cek status pelunasan di mutasi rekening.
                            </div>
                            <div>
                                <a href="{{ url('admin/sales/lunas/'.@$sale->id) }}" class="btn btn-success btn-sm m-0 no-underline"><i class="fa fa-check"></i> Lunas</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="row">
            <div class="col-12">
                <form action="{{ @$sale ? route('sales-online.update', $sale->id) : route('sales-online.store') }}" method="POST">
                    @csrf
                                                ...
                                                @foreach($sale->details as $key => $detail)
                                                    <tr id="item-id-{{ $detail->id }}">
                                                        ...
                                                        <td class="align-middle text-center">
                                                            ...
                                                            <form action="{{ url('admin/sales/delete/detail/'.$detail->id) }}" method="post" class="formDelete-{{ $detail->id }} d-none">
                                                                {!! csrf_field() !!}
                                                                {!! method_field('delete') !!}
                                                            </form> --}}
                                                        </td>
                        ...
                        <div class="col-4">
                            @if(@$sale && @$sale->status != 'BELUM LUNAS')
                                <div class="row">
                                    ...
                                    <div class="col-8 text-right">
                                        <strong>{{ $sale->buyer->name }}</strong><br>
                            ...
                            @else
                                <div class="d-flex align-items-center justify-content-between">
                                    ...
                                    {{-- <input type="text" readonly class="form-control-plaintext" id="staticEmail" value="{{ $sale->buyer->name }}"> --}}
                            ...
                            @if(@$sale)
                                @if(@$sale->status == 'BELUM LUNAS')
                                    ...
                                    <a href="{{ url('admin/sales/lunas/'.@$sale->id) }}" class="btn btn-block btn-success mb-2">
                                        <i class="fa fa-check"></i> Ubah status transaksi menjadi Lunas
                                    </a>
                                    ...
                                    <hr>
                                    <a href="{{ url('admin/sales/cancel/'.@$sale->id) }}" class="btn btn-block btn-secondary mt-3 mb-2">
                                ...
                                @elseif(@$sale->status == 'LUNAS')
                                    ...
                                    <a href="{{ url('admin/sales/dikirim/'.@$sale->id) }}" class="btn btn-block btn-primary btn- my-2">
                                        <i class="fa fa-check"></i> Barang telah dikirim ke {{ $sale->buyer->name }}
                                    </a>
                                    {{-- <a role="button" href="#" onclick="window.print()" class="btn btn-outline-link btn-block mt-2 mb-3">
                                        <i class="fa fa-print"></i> Cetak Nota Penjualan
                                    </a> --}}
                                @elseif(@$sale->status == 'DIKIRIM')
                                    ...
                                    <a href="{{ url('admin/sales/finish/'.@$sale->id) }}" class="btn btn-block btn-info my-2">
                                        <i class="fa fa-check"></i> Barang telah diterima {{ $sale->buyer->name }}
                                    </a>
                                    {{-- <a role="button" href="#" onclick="window.print()" class="btn btn-outline-link btn-block mt-2 mb-3">
                                        <i class="fa fa-print"></i> Cetak Nota Penjualan
                                    </a> --}}
